"""Device risk workflows with in-memory responses; no business database mutations."""
import csv
import io
import json
...
with sync_playwright() as p:
    ...
    reads=[];writes=[];held=[];errors=[];state={'fail':False,'hold':False,'read_fail':True,'read_hold':False}
    ...
    def fixture(route):
        ...
        reads.append((path,q))
        if path!='risk/devices':route.fulfill(json={'items':[],'total':0});return
    ...
    def query():return next(q for path,q in reversed(reads) if path=='risk/devices')
    def reset():form.locator('[type=reset]').click();ready()
    page.wait_for_function('document.querySelector("#whitelistError").textContent==="fixture unavailable"')
    assert table.locator('tbody tr,[data-white-edit],[data-white-toggle]').count()==0
    state['read_fail']=False;page.locator('#whitelistRefresh').click()
    ...
    count=len(reads);table.locator('[data-white-search=game_name]').first.click();page.wait_for_timeout(50)
    assert len(reads)==count and table.locator('[data-white-select]:checked').count()==1
    assert not form.is_visible() and form.locator('[data-filter-lookup=game_id]').count()==0
    page.locator('#whitelistSearch').click()
    form.locator('[data-filter-lookup=agent_id]').click();page.locator('.member-lookup-results:visible .sp_results li').first.click()
    form.evaluate('f=>f.requestSubmit()');ready(1);assert query()['agent_id']==['7100']
    reset()
    ...
    # Changes target member IDs. The injected list does not infer a device inclusion rule.
    target=table.locator('[data-white-toggle=is_white]').first;removed=int(target.get_attribute('data-white-id'));target.click();ready(225)
    ...
    page.wait_for_function('document.querySelector("#whitelistError").textContent==="fixture unavailable"');assert table.locator('tbody tr').count()==225
    state['read_fail']=False;page.locator('#whitelistRefresh').click();ready(225)
    ...
    for width in [1920,1024,390]:
        ...
        assert page.locator('#deviceRiskHost').evaluate('el=>el.scrollWidth<=el.clientWidth+1'),width
    page.locator('[data-action=cards]').click();assert table.locator('article').count()==1
    assert page.locator('#deviceRiskHost').evaluate('el=>el.scrollWidth<=el.clientWidth+1')
    page.screenshot(path='visual-baseline/verified/device-risk-mobile.png',full_page=True)
    page.locator('[data-action=cards]').click()
    ...
    page.evaluate("location.hash='risk-devices'");ready(1);assert form.is_visible() and 'closed failure' not in page.locator('body').inner_text()
    assert not errors,errors
    browser.close()
print('Device risk fixtures: selection/export, names, switches/retry, member/coin/behavior dialogs, All, stale scope and mobile passed')
