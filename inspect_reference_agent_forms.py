...
with sync_playwright() as p:
    ...
    response = page.request.get(BASE + '/agent/index', params={'limit':1,'offset':0,'sort':'id','order':'desc','filter':'{}','op':'{}'}, headers={'X-Requested-With':'XMLHttpRequest'})
    data = response.json()
    rows = data.get('rows', [])
    result = {'list_status': response.status, 'total':data.get('total'), 'forms':{}}
    suffix = f"/ids/{rows[0]['id']}" if rows else ''
    for action in (['ossedit','edit'] if rows else []):
        page.goto(BASE + f'/agent/{action}' + suffix, wait_until='domcontentloaded')
    ...
    output = Path('visual-baseline/reference-verified/agents-forms.json')
