...
from sqlalchemy import DateTime, Float, Integer, String, Text, and_, case, create_engine, func, inspect, or_, select, text
...
class AdRecord(Base, TimestampMixin):
    ...
    reward_type: Mapped[str] = mapped_column(String(32), default="棰嗗彇")
    ad_type: Mapped[str] = mapped_column(String(32), default="??")
    sub_ad_type: Mapped[str] = mapped_column(String(32), default="")
    ad_group: Mapped[str] = mapped_column(String(32), default="涓诲箍", server_default="涓诲箍")
    ...
    status: Mapped[str] = mapped_column(String(32), default="鎴愬姛")
...
class AdImportBatch(Base, TimestampMixin):
    ...
    status: Mapped[str] = mapped_column(String(32), default="瀹屾垚")
...
AD_EXPORT_COLUMNS = [("ID","id"),("??ID","parent_id"),("??ID","user_id"),("????","user_account"),("?????","receive_name"),("????","game_name"),("ECPM","ecpm"),("??","coin"),("????","ad_network_platform_name"),("????","reward_type"),("????","ad_group"),("??","status"),("????","watched_at"),("?????","ad_code")]
AD_STATS_GROUP_LABELS={"day":"??","ad_group":"????","game":"??","agent":"???"}
AD_IMPORT_FIELD_ALIASES={k:k for k in ("id","parent_id","user_id","user_account","receive_name","agent_id","game_id","game_name","ecpm","coin","estimate_income","ad_network_platform_name","reward_type","ad_group","status","watched_at","ad_code")}
...
def list_payload(
    ...
    rows = session.execute(stmt.order_by(model.id.desc()).offset(offset).limit(limit)).scalars().all()
...
def serialize_game(item: Game) -> dict[str, Any]:
    return serialize(item, GAME_FIELDS)
...
def decode_ad_import_body(body: bytes) -> str:
    if not body.strip():
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_CONTENT, detail="瀵煎叆鏂囦欢涓嶈兘涓虹┖")
    for encoding in ("utf-8-sig", "utf-8", "gb18030"):
        try:
            return body.decode(encoding)
        except UnicodeDecodeError:
            continue
    raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_CONTENT, detail="閻庣數鍘ч崣鍡涘棘閸ワ附顐界紓鍌涚墱閻栨粓妫侀埀顑胯厬UTF-8 闁?GBK")
...
def parse_ad_import_rows(body: bytes) -> list[dict[str, Any]]:
    ...
    try:
        ...
        if not reader.fieldnames:
            raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_CONTENT, detail="瀵煎叆鏂囦欢缂哄皯琛ㄥご")
        return list(reader)
    except csv.Error as exc:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_CONTENT, detail="CSV 鏍煎紡鏃犳晥") from exc
...
def normalize_ad_import_row(raw_row: dict[str, Any], row_number: int, seen_request_ids: set[str]) -> tuple[dict[str, Any] | None, list[str]]:
    ...
    for raw_key, raw_value in raw_row.items():
        ...
        if field in AD_IMPORT_STRING_FIELDS:
            ...
            try:
                ...
                errors.append(f"{key}涓嶆槸鏈夋晥鏁板瓧")
        elif field == "watched_at":
            try:
                payload[field] = parse_ad_import_datetime(value)
            except ValueError:
                errors.append("瑙傜湅鏃堕棿鏍煎紡鏃犳晥")

    request_id = str(payload.get("request_id") or "").strip()
    if not request_id:
        errors.append("request_id涓嶈兘涓虹┖")
    elif request_id in seen_request_ids:
        errors.append("request_id闁革负鍔岄閬嶅礂閵夛附鐎ù鐘虫构閼垫垿鏌屽鍜佹Щ")
    else:
        seen_request_ids.add(request_id)

    ad_group = str(payload.get("ad_group") or "").strip()
    if ad_group and ad_group not in (MAIN_AD_LABEL, SECONDARY_AD_LABEL):
        errors.append("骞垮憡鍒嗙粍闁告瑯浜ｉ崗姗€寮伴娆忕槣妤犵偞瀵ч崹銊╁礈椤栨氨鐣?)
    ...
    payload.setdefault("status", "鎴愬姛")
    payload.setdefault("reward_type", "棰嗗彇")
    payload.setdefault("ad_type", "??" if payload["ad_group"] == MAIN_AD_LABEL else SECONDARY_AD_LABEL)
    payload.setdefault("sub_ad_type", "")
    return payload, []
...
def make_ad_alert(
    *,
    ...
    if source_label:
        ...
        source_label_value = f"閻庣數鍘ч崣鍡涘箥鐟欏嫷鍋?#{source_id}"
    else:
        source_label_value = f"骞垮憡璁板綍 #{source_id}"
...
def build_ad_alerts(
    ...
    success_conditions = [*record_conditions, AdRecord.status == "鎴愬姛"]
    ...
    for row in session.execute(duplicate_stmt).all():
        alerts.append(
            make_ad_alert(
                ...
                severity="high",
                summary=f"request_id {row.request_id} ?? {row.occurrence_count} ?",
    ...
    for row in session.execute(mismatch_stmt.order_by(AdRecord.id.desc()).limit(120)).scalars().all():
        alerts.append(
            make_ad_alert(
                ...
                severity="low",
                summary=f"骞垮憡鍒嗙粍濞戞挸姘﹂～澶愬礆濞嗘劕鑵归柡鍌ゅ幒缁楀绋夐埀顒勬嚊鏉堝墽绀夌憸鐗堟尭婢х娀宕愰梻纾嬬 {row.ad_group or '-'}",
                source_type="ad_record",
                source_id=row.id,
                created_at=row.created_at,
                ad_group=classify_ad_group(row),
    ...
    for row in session.execute(import_stmt.order_by(AdImportError.created_at.desc()).limit(120)).all():
        ...
        severity = "high" if "闂佹彃绉撮ˇ? in message or "涓嶈兘涓虹┖" in message or "宸插瓨鍦╥n message else "medium"
        alerts.append(
            make_ad_alert(
                alert_type="import_error",
                severity=severity,
                summary=message or "瀵煎叆澶辫触",
...
def summarize_ad_alerts(alerts: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "source_breakdown": [
            {"source_type": source_type, "source_type_label": "骞垮憡璁板綍" if source_type == "ad_record" else "閻庣數鍘ч崣鍡涘箥鐟欏嫷鍋?, "count": count}
...
def require_nonblank(changes: dict[str, Any], field: str, label: str) -> None:
    if field in changes and (changes[field] is None or not str(changes[field]).strip()):
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_CONTENT, detail=f"{label}涓嶈兘涓虹┖")
    if field in changes:
...
def require_not_none(changes: dict[str, Any], fields: list[str]) -> None:
    ...
    if invalid is not None:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_CONTENT, detail=f"{invalid}濞戞挸绉烽崗妯硅厬null")
...
def validate_json_text(changes: dict[str, Any], field: str) -> None:
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_CONTENT, detail=f"{field}闊洤鎳橀妴蹇涘及椤栨稒绠掗柡?JSON") from exc
...
def require_admin(
    ...
    if credentials is None or credentials.scheme.lower() != "bearer":
        raise authentication_error("璇峰厛鐧诲綍")
    try:
    ...
    with SessionLocal() as session:
        ...
        if admin is None or admin.status != 1:
            raise authentication_error("绠＄悊鍛樹笉瀛樺湪鎴栧凡鍋滅敤")
...
def validate_agent_parent(session: Session, parent_id: int, agent_id: int | None = None) -> None:
    ...
    current = get_or_404(session, Agent, parent_id, "涓婄骇涓讳綋")
...
'        phone_admin = session.scalar(select(AdminUser).where(AdminUser.username == "18532306918"))
        if phone_admin is None:
            ...
            session.add(AdminUser(username="18532306918", password_hash=phone_hash, password_salt=phone_salt, display_name="娑撳娓ょ敮鍌涙噰缂囶垰楠囩純鎴犵捕缁夋垶濡ч張澶愭閸忣剙寰?, role="superadmin", status=1))
            session.commit()

'        if session.scalar(select(func.count()).select_from(Agent)) or 0:
            session.commit()
            return
        ...
        ad = AdRecord(
            ...
            coin=0.27,
            estimate_income=0.0,
            ad_network_platform_name="鑴夌洘",
            is_lottery=1,
            is_rw=0,
            reward_type="棰嗗彇",
            ad_type="??",
            sub_ad_type="",
            ad_group=MAIN_AD_LABEL,
            is_type=0,
            is_fu=0,
            fu_type=1,
            is_look=1,
        ...
        remark="?????")
        tags="", hardware_main_id="", ip="", action="login", risk_score=0, risk_level="low")
...
app = FastAPI(title="骞垮憡浼氬憳鍚庡彴", version="0.1.0", lifespan=lifespan)
...
def change_password(payload: PasswordChangeRequest, admin: AdminUser = Depends(require_admin)) -> Response:
    ...
    with SessionLocal() as session:
        ...
        if not verify_password(payload.current_password, stored_admin.password_hash, stored_admin.password_salt):
            raise authentication_error("鐟滅増鎸告晶鐘碘偓闈涙閻栨粓鏌ㄥ▎鎺濆殩")
...
def dashboard_registrations(days: int = Query(7, ge=1, le=31)) -> dict[str, Any]:
    ...
    with SessionLocal() as session:
        rows = session.execute(
            select(func.date(Member.created_at).label("day"), func.count(Member.id).label("count"))
            .where(Member.created_at >= datetime.combine(start, datetime.min.time(), tzinfo=UTC))
            .group_by(func.date(Member.created_at))
            .order_by(func.date(Member.created_at))
        ).all()
        counts = {str(row.day): int(row.count or 0) for row in rows}
            ...
            rows.append(
                {
                    "subject": "鎻愮幇闃熷垪",
                    "module": "withdrawal",
                    "action": f"闁?{pending_withdrawals} 鏉″緟瀹℃牳璁板綍" if pending_withdrawals else "褰撳墠鏃犲緟瀹″崟",
                    pass
...
def list_agents(
    ...
    with SessionLocal() as session:
        payload = list_payload(session, Agent, ["name", "user_name", "security_key"], serialize_agent, q, limit, offset, conditions)
...
def list_games(
    ...
    if parent_id is not None:
        conditions.append(Member.parent_id == parent_id)
    if agent_id is not None:
    ...
    with SessionLocal() as session:
        payload = list_payload(session, Game, ["name", "game_key", "game_url", "other_url"], serialize_game, q, limit, offset, conditions)
        payload["summary"] = summarize_flags(
            session,
            Game,
            conditions,
...
def list_members(
    ...
    with SessionLocal() as session:
        ...
        # Compatibility aliases used by the target member table.
        for row in payload.get("items", []):
            row.setdefault("parent_name", row.get("parent_id") or "")
            row.setdefault("game_name", row.get("game_id") or "")
            row.setdefault("agent_name", row.get("agent_id") or "")
            row.setdefault("receive_name", "")
        payload["summary"] = summarize_flags(
            session,
            Member,
            conditions,
...
async def import_ads(request: Request, admin: AdminUser = Depends(allow_roles("operator"))) -> dict[str, Any]:
    ...
    for index, raw_row in enumerate(raw_rows, start=2):
        ...
        if row_errors:
            ...
            error["request_id"] = str(raw_row.get("骞垮憡request_id") or raw_row.get("request_id") or "").strip()
    ...
    file_name = unquote(request.headers.get("x-file-name", "")).strip() or "闁哄牜浜滈幊鈥虫儊csv"
    with SessionLocal() as session:
        batch = AdImportBatch(
        ...
        for row_number, payload in valid_rows:
            if payload["request_id"] in existing_ids:
                error = "".join(error.get("errors") or [])[:255],
        ...
        batch.status = "瀹屾垚" if batch.rejected == 0 else "閮ㄥ垎澶辫触" if batch.accepted else "澶辫触"
        session.commit()
        batch_id = batch.id
...
def list_ad_import_errors(
    ...
    with SessionLocal() as session:
        batch = get_or_404(session, AdImportBatch, batch_id, "閻庣數鍘ч崣鍡涘箥鐟欏嫷鍋?)
...
def list_withdrawals(
    ...
    with SessionLocal() as session:
        ...
        conditions = [Withdrawal.status == internal_status] if internal_status is not None else None
        payload = list_payload(
            session,
            Withdrawal,
...
def _batch_withdrawal_update(ids: list[int], target_status: int | None = None, plan_status: int | None = None, reason: str = "", admin: AdminUser | None = None) -> dict[str, Any]:
    if not ids:
        detail="??????")
    with SessionLocal() as session:
        ...
        for item in rows:
            if target_status is not None:
                if item.status != 0:
                    detail="??????")
                if target_status == 2 and not reason.strip():
                    raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_CONTENT, detail="椹冲洖鍘熷洜涓嶈兘涓虹┖")
            ...
            if plan_status is not None:
                if item.status != 1 or item.plan_status != 0:
                    raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="闁告瑯浜ｉ崗妯绘姜椤掑啫顦╃€瑰憡褰冮鎼佸冀闂€鎰懍闁哄牜浜滈ˇ鈺呮偠閸℃瑦鐣辨彁鐜拌褰?)
...
def batch_transfer_withdrawals(payload: WithdrawalBatch, admin: AdminUser = Depends(allow_roles("reviewer"))) -> dict[str, Any]:
    return _batch_withdrawal_update(payload.ids, plan_status=1, admin=admin)
...
def list_subsidies(
    ...
    with SessionLocal() as session:
        internal_status = 2 if status_filter == 4 else status_filter
        conditions = [Subsidy.status == internal_status] if internal_status is not None else None
        return list_payload(session, Subsidy, ["sub_msg", "pics"], serialize_subsidy, q, limit, offset, conditions)
...
def _batch_subsidy_update(ids: list[int], target_status: int, message: str, admin: AdminUser) -> dict[str, Any]:
    if not ids:
        detail="??????")
    if target_status == 2 and not message.strip():
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_CONTENT, detail="椹冲洖鍘熷洜涓嶈兘涓虹┖")
    with SessionLocal() as session:
...
def list_coin_logs(
    ...
    with SessionLocal() as session:
        ...
        payload["summary"] = {"change": float(session.scalar(select(func.coalesce(func.sum(CoinLog.coin), 0.0)).where(*conditions)) or 0)}
        return payload


@api.get("/risk/history")
def list_risk_history(q: str | None = None, risk_level: str | None = None, action: str | None = None, limit: int = Query(20, ge=1, le=200), offset: int = Query(0, ge=0)) -> dict[str, Any]:
    with SessionLocal() as session:
        conditions: list[Any] = []
        if risk_level: conditions.append(RiskRecord.risk_level == risk_level)
        if action: conditions.append(RiskRecord.action == action)
        return list_payload(session, RiskRecord, ["tagcode", "tags", "hardware_main_id", "ip", "action"], serialize_risk, q, limit, offset, conditions)


@api.get("/risk/whitelist")
def list_whitelist(q: str | None = None, status_filter: int | None = Query(None, alias="status"), limit: int = Query(20, ge=1, le=200), offset: int = Query(0, ge=0)) -> dict[str, Any]:
    with SessionLocal() as session:
        conditions = [Member.is_white == 1]
        if status_filter is not None: conditions.append(Member.status == status_filter)
        stmt = select(Member).where(*conditions)
        if q:
            stmt = stmt.where(or_(Member.username.ilike(f"%{q}%"), Member.name.ilike(f"%{q}%"), Member.device_id.ilike(f"%{q}%")))
        count_stmt = select(func.count()).select_from(stmt.subquery())
        total = session.scalar(count_stmt) or 0
        rows = session.execute(stmt.order_by(Member.id.desc()).offset(offset).limit(limit)).scalars().all()
        return {"total": total, "items": [serialize_member(row) for row in rows], "limit": limit, "offset": offset}
...
def list_device_risk(q: str | None = None, limit: int = Query(20, ge=1, le=200), offset: int = Query(0, ge=0)) -> dict[str, Any]:
    with SessionLocal() as session:
        return list_payload(session, RiskRecord, ["hardware_main_id", "ip"], serialize_risk, q, limit, offset)


@api.get("/agents/{agent_id}")
def get_agent(agent_id: int) -> dict[str, Any]:
    with SessionLocal() as session:
        return serialize_agent(get_or_404(session, Agent, agent_id, "涓讳綋"))
...
def create_agent(payload: AgentCreate) -> dict[str, Any]:
    ...
    require_nonblank(changes, "name", "涓讳綋鍚嶇О")
    with SessionLocal() as session:
...
def update_agent(agent_id: int, payload: AgentUpdate) -> dict[str, Any]:
    ...
    with SessionLocal() as session:
        item = get_or_404(session, Agent, agent_id, "涓讳綋")
...
def get_game(game_id: int) -> dict[str, Any]:
    with SessionLocal() as session:
        return serialize_game(get_or_404(session, Game, game_id, "娓告垙"))
...
def create_game(payload: GameCreate) -> dict[str, Any]:
    changes = payload.model_dump()
    require_nonblank(changes, "name", "娓告垙鍚嶇О")
...
def update_game(game_id: int, payload: GameUpdate) -> dict[str, Any]:
    ...
    validate_json_text(changes, "settings_json")
    with SessionLocal() as session:
        item = get_or_404(session, Game, game_id, "娓告垙")
...
def get_member(member_id: int) -> dict[str, Any]:
    with SessionLocal() as session:
        return serialize_member(get_or_404(session, Member, member_id, "浼氬憳"))
...
def create_member(payload: MemberCreate) -> dict[str, Any]:
    changes = payload.model_dump()
    require_nonblank(changes, "username", "浼氬憳璐﹀彿")
    with SessionLocal() as session:
...
def update_member(
    ...
    with SessionLocal() as session:
        item = get_or_404(session, Member, member_id, "浼氬憳")
...
def delete_member(member_id: int) -> Response:
    with SessionLocal() as session:
        ...
        if any(has_rows(session, model, condition) for model, condition in dependencies):
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="浼氬憳濞寸姴绉靛﹢渚€宕楃€圭姳绮撴繛缈犵劍閹稿骞嬮弽褜鍚€闁哄秵鐡曢鍥亹閺囶亞绀夊☉鎾崇Х閸忔﹢宕氶悩缁樼彑")
        session.delete(item)
        session.commit()
        return Response(status_code=status.HTTP_204_NO_CONTENT)
...
def update_subsidy(
    ...
    changes = payload.model_dump(exclude_unset=True)
    require_not_none(changes, ["status"])
    if "status" in changes and changes["status"] not in (0, 1, 2, 4):
        detail="??????")
    with SessionLocal() as session:
        item = get_or_404(session, Subsidy, subsidy_id, "琛ヨ创璁板綍")
        if "status" in changes and changes["status"] != item.status:
            if item.status != 0:
                detail="??????")
            if changes["status"] in (1, 2, 4):
...
def resource_detail(resource: str, item_id: int) -> dict[str, Any]:
    ...
    with SessionLocal() as session:
        return serializer(get_or_404(session, model, item_id, "璁板綍"))
