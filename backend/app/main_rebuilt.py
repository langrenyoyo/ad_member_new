...
def summarize_ad_alerts(alerts: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "source_breakdown": [
            {"source_type": source_type, "source_type_label": "广告记录" if source_type == "ad_record" else "导入批次", "count": count}
...
def validate_agent_parent(session: Session, parent_id: int, agent_id: int | None = None) -> None:
    ...
    while current is not None:
        if current.id in visited:
            raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_CONTENT, detail="主体层级不能形成循环")
...
def validate_member_scope(session: Session, agent_id: int, game_id: int) -> None:
    ...
    if game_id:
        if agent_id and game.agent_id != agent_id:
            raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_CONTENT, detail="参数校验失败")
...
def seed_data() -> None:
    ...
    with SessionLocal() as session:
        ...
        if phone_admin is None:
            ...
            session.add(AdminUser(username="18532306918", password_hash=phone_hash, password_salt=phone_salt, display_name="管理员", role="superadmin", status=1))
            session.commit()
...
def _batch_withdrawal_update(ids: list[int], target_status: int | None = None, plan_status: int | None = None, reason: str = "", admin: AdminUser | None = None) -> dict[str, Any]:
    ...
    with SessionLocal() as session:
        ...
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="记录状态不允许此操作")
