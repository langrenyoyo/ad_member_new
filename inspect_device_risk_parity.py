"""Read-only device-risk inspection with synthetic browser responses."""
import ast
import json
...
OUT=Path('visual-baseline/fixtures/device-risk');OUT.mkdir(parents=True,exist_ok=True)
...
rows=[{'id':8100+i,'username':['fixture 001','fixture <>&','fixture 003'][i],'image_url':'','parent_id':8000,
       ...
       'is_white':i%2,'status':1-i%2,'game_addiction_enable':i%2,'game_addiction_time':'2026-09-18 08:00:00',
...
with sync_playwright() as p:
    ...
    def readonly(route):
        ...
        if request.method not in ('GET','HEAD') or any(part in url.path for part in ('/lahei','/multi','/del','/agree','/refuse','/alipay','/ban','/coinclear')):
            route.abort();return
        if '/risk/userd' in url.path and ('sort' in q or request.headers.get('x-requested-with')=='XMLHttpRequest'):
            requests.append(q);route.fulfill(json={'total':3,'rows':rows});return
        route.continue_()
    page.route('**/*',readonly)
    page.goto(BASE+'/risk/userd',wait_until='networkidle')
    ...
    response=page.request.get('https://ad.leadink.cn/assets/js/backend/risk/userd.js')
    ...
    game_requests_before=len(requests)
    page.locator('#table tbody .searchit[data-field="game.name"]').first.click();page.wait_for_function('jQuery.active===0')
    report['game_search']={'requests_added':len(requests)-game_requests_before,'selected':page.evaluate("jQuery('#table').bootstrapTable('getSelections').length"),'filters':page.locator('.form-commonsearch input').evaluate_all('nodes=>nodes.map(n=>({name:n.name,value:n.value,type:n.type}))')}
    page.evaluate("jQuery('#table').bootstrapTable('uncheckAll')")
