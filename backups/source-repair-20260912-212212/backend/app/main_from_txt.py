...
AD_EXPORT_COLUMNS = [('ID', 'id'), ('上级ID', 'parent_id'), ('上级支付宝姓名', 'parent_payment_name'), ('会员ID', 'user_id'), ('用户账号', 'user_account'), ('支付宝姓名', 'receive_name'), ('游戏名称', 'game_name'), ('ECPM', 'ecpm'), ('金币', 'coin'), ('广告平台', 'ad_network_platform_name'), ('奖励类型', 'reward_type'), ('广告分组', 'ad_group'), ('状态', 'status'), ('观看时间', 'watched_at'), ('广告代码位', 'ad_code'), ('广告request_id', 'request_id'), ('交易trans_id', 'trans_id')]
...
def profile_operations(q: str = "", limit: int = Query(10, ge=1, le=100), offset: int = Query(0, ge=0), admin: AdminUser = Depends(require_admin)) -> dict[str, Any]:
    with SessionLocal() as session:
...
def list_members(
    ...
    if ip:
        conditions.append(Member.ip.contains(ip, autoescape=True))
    if create_time:
        try:
            ...
            if len(dates) > 2 or end <= start:
                raise ValueError
        except ValueError:
            raise HTTPException(status_code=422, detail="创建时间请使用 YYYY-MM-DD 或 YYYY-MM-DD - YYYY-MM-DD")
        conditions.extend([Member.created_at >= start, Member.created_at < end])
    if agent_name:
    ...
    if game_name:
        conditions.append(Member.game_id.in_(select(Game.id).where(Game.name.contains(game_name, autoescape=True))))
...
def list_coin_logs(
    ...
    with SessionLocal() as session:
        ...
        agents = dict(session.execute(select(Agent.id, Agent.name).where(Agent.id.in_({row["agent_id"] for row in rows}))).all())
        for row in rows:
            row.update(username=users.get(row["user_id"], ""), game_name=games.get(row["game_id"], ""), agent_name=agents.get(row["agent_id"], ""))
        return payload
...
def list_risk_history(q: str | None = None, risk_level: str | None = None, action: str | None = None,
                      ...
                      user_id: str | None = None, username: str | None = None, parent_id: int | None = None,
                      game_id: int | None = None, agent_id: int | None = None, tagcode: str | None = None,
    ...
    with SessionLocal() as session:
        ...
        users = dict(session.execute(select(Member.id, Member.username).where(Member.id.in_({row["user_id"] for row in rows}))).all())
        games = dict(session.execute(select(Game.id, Game.name).where(Game.id.in_({row["game_id"] for row in rows}))).all())
        for row in rows: row.update(username=users.get(row["user_id"], ""), game_name=games.get(row["game_id"], ""))
        return payload
...
def list_whitelist(q: str | None = None, username: str | None = None, name: str | None = None,
                   ...
                   created_from: datetime | None = None, created_to: datetime | None = None,
                   status_filter: int | None = Query(None, alias="status"), limit: int = Query(20, ge=1, le=200), offset: int = Query(0, ge=0)) -> dict[str, Any]:
    with SessionLocal() as session:
        conditions = [Member.is_white == 1]
        if status_filter is not None: conditions.append(Member.status == status_filter)
        if username: conditions.append(Member.username.contains(username, autoescape=True))
        if name: conditions.append(Member.name.contains(name, autoescape=True))
        if parent_id is not None: conditions.append(Member.parent_id == parent_id)
        if game_id is not None: conditions.append(Member.game_id == game_id)
        if agent_id is not None: conditions.append(Member.agent_id == agent_id)
        if created_from is not None: conditions.append(Member.created_at >= created_from)
        if created_to is not None: conditions.append(Member.created_at <= created_to)
        ...
        games = dict(session.execute(select(Game.id, Game.name).where(Game.id.in_({row.game_id for row in rows}))).all())
        parents = dict(session.execute(select(Member.id, Member.name).where(Member.id.in_({row.parent_id for row in rows}))).all())
        items = [dict(serialize_member(row), game_name=games.get(row.game_id, ""), parent_name=parents.get(row.parent_id, "")) for row in rows]
...
def list_device_risk(q: str | None = None, limit: int = Query(20, ge=1, le=200), offset: int = Query(0, ge=0)) -> dict[str, Any]:
    with SessionLocal() as session:
        payload = list_payload(session, RiskRecord, ["hardware_main_id", "ip"], serialize_risk, q, limit, offset)
        rows=payload["items"]
        users=dict(session.execute(select(Member.id,Member.username).where(Member.id.in_({r["user_id"] for r in rows}))).all())
        for row in rows: row["username"]=users.get(row["user_id"],"")
        return payload
...
def game_lottery_records(
    ...
    with SessionLocal() as session:
        game = get_or_404(session, Game, game_id, "游戏")
        conditions = [LotteryRecord.game_id == game_id]
    ...
    with SessionLocal() as session:
        ...
        for value in login_candidates:
            try:
                ...
                if parsed.astimezone(UTC).date() == beijing_day: today_login += 1
        ...
        income = session.execute(select(func.date(AdRecord.created_at), func.coalesce(func.sum(AdRecord.estimate_income), 0.0)).where(AdRecord.game_id == game_id).group_by(func.date(AdRecord.created_at)).order_by(func.date(AdRecord.created_at))).all()
        return {"game_id": game_id, "total_members": int(total_members), "today_new": int(today_new), "today_login": int(today_login),
                "total_income": float(total_income), "year_income": income_between(year_start, day_end), "month_income": income_between(month_start, day_end), "yesterday_income": income_between(yesterday_start, day_start),
                "activity": [{"date": str(day), "num": int(num)} for day, num in activity],
                "income": [{"date": str(day), "amount": float(amount)} for day, amount in income]}
...
def game_login_logs(
    ...
    with SessionLocal() as session:
        game=get_or_404(session,Game,game_id,"游戏")
        conditions=[MemberLoginLog.game_id==game_id]
        ...
        for row in payload['items']:row.update(username=users.get(row['user_id'],''),game_name=game.name)
        return payload
...
def get_agent(agent_id: int) -> dict[str, Any]:
    with SessionLocal() as session:
        return serialize_agent(get_or_404(session, Agent, agent_id, "主体"))
...
def get_game(game_id: int) -> dict[str, Any]:
    with SessionLocal() as session:
        return serialize_game(get_or_404(session, Game, game_id, "游戏"))
